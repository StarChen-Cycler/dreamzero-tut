...
- `observation`: what the robot senses, usually images plus state
- `action`: motor command sent to the robot
- `policy`: a mapping from context to action
- `closed-loop control`: the robot acts, sees the new state, then acts again
- `embodiment`: the robot's physical body and actuation layout

## Minimal Modeling Vocabulary

- `VLM`: a vision-language model, strong at semantics
- `VLA`: a vision-language-action model, predicts actions from visual and language inputs
- `world model`: a model that predicts how the world will evolve
- `WAM`: a world action model, here meaning a model that predicts future observations and actions jointly
- `diffusion model`: a model trained to reverse a noise process
